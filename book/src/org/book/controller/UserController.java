...
public class UserController {
    ...
    public String login(String uname, String pwd, String remember, HttpSession session, HttpServletResponse resp, HttpServletRequest req) throws IOException {
        ...
        if (user != null){
            ...
            if ("remember".equals(remember)){
                ...
                //设置存活时间：30min
                un_cookie.setMaxAge(60*30);
                p_cookie.setMaxAge(60*30);
                //将cookie保存在浏览器端
    ...
    //修改用户
    public String editUser(Integer id,String uname,String pwd,HttpSession session){
        userService.editUser(id,uname,pwd);
        User user = userService.login(uname, pwd);
        session.setAttribute("user",user);
        return "user/person";
    }
